...
def update_basic_ticker_data():
    ...
    if BasicTickerData.objects.get(symbol="AAPL").updated_at == date.today():
        ...
        try:
            ...
            print(f"Failed to fetch basic ticker data: {e}")
            return
        ...
        print("Basic ticker data updated successfully.")

def update_favorite_detailed_and_historical_data():
    ...
    for ticker in FavoriteTickerData.objects.all():
        if DetailedTickerData.objects.filter(basic_data=ticker.basic_data).exists() and DetailedTickerData.objects.get(basic_data=ticker.basic_data).updated_at == date.today():
            ...
            print(f"Updating detailed ticker data for {ticker.basic_data.symbol}...")
            try:
                ...
                # Handle request errors
                print(f"Failed to fetch detailed ticker data for {ticker.basic_data.symbol}: {e}")
    ...
    print("Favorite detailed and historical ticker data updated successfully.")

def update_general_news():
    ...
    if GeneralNews.objects.all().count() > 0 and GeneralNews.objects.all()[0].updated_at == date.today():
        print("General news updated successfully with no changes.")
    else:
        print("Updating general news...")
        try:
